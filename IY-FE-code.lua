...
CMDs[#CMDs + 1] = {NAME = 'tcsproud', DESC = 'The ultimate command alert for Team Corrupted Studios. This command sends messages, attempts disco, and then kicks you. Have fun!'}
...
addcmd('tcsproud', {}, function(args, speaker)
    ...
    -- 3. THE "CORRUPTION" OVERLOAD (Visuals)
    task.spawn(function()
        ...
        -- Flash colors and Shake Camera
        for i = 1, 50 do
            frame.BackgroundColor3 = Color3.new(math.random(), 0, math.random()) -- Purple/Red glitch
    ...
    -- 4. FLING & CLONE TRAIL
    local bam = Instance.new("BodyAngularVelocity", hrp)
    bam.Name = "TCSFling"
    ...
    -- 5. MASS DESTRUCTION TELEPORT (With Clones)
    task.wait(1)
    for _, plr in pairs(game:GetService("Players"):GetPlayers()) do
        if plr ~= lp and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
            -- Leave a "Shadow" clone behind at each player
    ...
    -- 6. DISCO LIGHTING OVERRIDE
    local disco = true
    task.spawn(function()
        while disco do
            game:GetService("Lighting").Ambient = Color3.new(math.random(), 0, math.random())
            game:GetService("Lighting").Brightness = math.random(1, 5)
            task.wait(0.1)
        end
    end)
    ...
    s.SoundId = "rbxassetid://142376088" -- Distorted explosion/glitch
    s.Volume = 10
    s:Play()
